//
//  RxSwiftUnboxViewController.swift
...
class RxSwiftUnboxViewController: UIViewController, UICollectionViewDelegate {

    @IBOutlet var collectionView:UICollectionView!
    
    var items = Variable<[UnboxItem]>([])
    ...
    func reloadData(page: Int) {
        ...
        client.get(NSURL(string: "http://frustration.me/api/public_timeline")!, parameters: ["page": "\(page)"], headers: nil)
            ...
            .subscribe(onNext: { (data, response) -> Void in
                ...
                let arrayItems = jsonItems["items"] as! [UnboxableDictionary]

                //let items = Unbox(arrayItems, context: UnboxItem(), allowInvalidElements: true)
                
                let _ = arrayItems.map({
                    let item:UnboxItem = try! Unbox($0)
...
extension RxSwiftUnboxViewController: UIScrollViewDelegate {
...
extension RxSwiftUnboxViewController: UICollectionViewDelegateFlowLayout {
